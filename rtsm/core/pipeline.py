...
class Pipeline:
    ...
    def run_one_step(self):
        ...
        # snap.rgb is actually BGR despite the name — see rtsm/io/websocket.py
        # decode_rgb(), which emits BGR from both BGRA and NV12 paths.
        # Flip once here at the source so segmentation (GDINO), CLIP, and
        # any downstream consumer all receive correct RGB.
        if isinstance(snap.rgb, np.ndarray):
        ...
        for c in cands:
            ...
            if c.mask is not None:
                ...
                if mask_region.numel() > 0:
                    ...
                    # Default is "none": CLIP was trained on natural photos with
                    # full context; masked cutouts are out-of-distribution and
                    # collapse embeddings to a noise floor. Pass the unmasked
                    # bbox crop through instead — the padding gives CLIP local
                    # context, and the bbox tightness keeps the object dominant.
                    bg_fill = self.cfg.get("staging", {}).get("bg_fill", "none")
                    if bg_fill == "none":
                        pass  # no masking; use the raw RGB bbox crop
                    elif bg_fill == "white":
    ...
    def _clip_batch_inplace(self, cands: List[Candidate]):
        ...
        for i, c in enumerate(cands):
            ...
            # Dump first 20 crops (RGB-corrected) so we can see exactly
            # what CLIP is being fed.
            _cnt = getattr(self, "_dbg_crop_counter", 0)
            if _cnt < 20:
                import os as _os
                _dump_dir = "/tmp/rtsm_crops"
                _os.makedirs(_dump_dir, exist_ok=True)
                from PIL import Image as _PILImage
                _PILImage.fromarray(crop_rgb).save(f"{_dump_dir}/crop_{_cnt:03d}.png")
                self._dbg_crop_counter = _cnt + 1
                if _cnt == 0:
                    logger.info(f"[dump-crops] writing to {_dump_dir}")
            imgs.append(Image.fromarray(crop_rgb))  # already 224x224 uint8, now RGB
            idxs.append(i)
        if not imgs:
            return
